...
public class Oview extends SurfaceView implements Runnable,
    ...
    private int bgBitmapHeight = 720;
    private int bgBitmapWidth = 640;

    // 当前的已经选择的画笔参数
    private int currentPaintTool = 0; // 0~10
    ...
    public Oview(Context context, AttributeSet arr) {
        super(context, arr);
    ...
    // 检测点击事件，是否在画笔类型选择区域
    public boolean selected(int se) {
        currentPaintTool = se;
        return true;
    }
...
public class Paletteoview extends SurfaceView implements Runnable,
		SurfaceHolder.Callback {

	private Paint mPaint = null;
	// 画板的坐标以及宽高
	private int bgBitmapX = 80;
	private int bgBitmapY = 0;
	private int bgBitmapHeight = 480;
	private int bgBitmapWidth = 640;
	// 画笔选项工具栏的坐标,宽高,每行和每列小框个数
	private int toolsX = 0;
	private int toolsY = 0;
	private int toolsSide = 80;
	private int toolsHeightNum = 6;
	private int toolsWidthNum = 1;
	// 笔粗细大小工具栏坐标，宽高
	private int sizeX = bgBitmapX+bgBitmapWidth;
	private int sizeY = 0;
	...
	// 已选选择单元背景色
	private int selectedCellColor = Color.RED;
	// 存储所有的动作
	private ArrayList<Action> actionList = null;
	// 当前的画笔实例
	private Action curAction = null;
	...
	public Paletteoview(Context context, AttributeSet arr) {
		super(context, arr);
		...
		bgBitmap = ((BitmapDrawable) (getResources()
				.getDrawable(R.drawable.whirte))).getBitmap();
		newbit = Bitmap.createBitmap(bgBitmapWidth, bgBitmapHeight,
				Config.ARGB_4444);
		new Thread(this).start();
	}
	...
	private void drawToolsText(Canvas canvas, int cellX, int cellY, int cellBX,
		...
		switch (paintTool) {
		case 0:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "select");
			break;
		case 1:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "pen");
			break;
		case 2:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "rubber");
			break;
		case 3:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "word");
			break;
		case 4:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "photo");
			break;
		case 5:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "express");
			break;
		case 6:
	...
	private void drawSizeText(Canvas canvas, int cellX, int cellY, int cellBX,
		...
		switch (toolSize) {
		case 1:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "start");
			break;
		case 3:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "stop");
			break;
		case 5:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "00:00");
			break;
		case 7:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "record");
			break;
		case 9:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "movie");
			break;
		case 11:
			drawCellText(canvas, cellX, cellY, cellBX, cellBY, "return");
			break;
		}
	}
	...
	private void drawCellText(Canvas canvas, int cellX, int cellY, int cellBX,
		...
		paint.setColor(Color.BLACK);
		paint.setTextSize((cellBY - cellY) / 5);
		int textX = cellX + (cellBX - cellX) / 5;
		int textY = cellBY - (cellBY - cellY) / 2;
